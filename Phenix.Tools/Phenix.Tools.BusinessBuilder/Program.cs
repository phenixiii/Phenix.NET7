...
{{
    ...
    {{
        ...
        public {7}(string dataSourceKey, long id, bool? isNew, bool? isSelfDeleted, bool? isSelfDirty, IDictionary<string, object> oldPropertyValues, IDictionary<string, bool?> dirtyPropertyNames,
            ",
            ...
            foreach (KeyValuePair<string, Column> kvp in sheet.Columns)
                if (!String.IsNullOrEmpty(kvp.Value.DataDefault) &&
                    ...
                    try
                    {
                        codeBuilder.Append(String.Format(@"
            {0} = {1}{2}{1};",
                            kvp.Value.FieldName, kvp.Value.MappingType == typeof(string) ? "\"" : null,
                            Utilities.ChangeType(Utilities.ChangeType(kvp.Value.DataDefault, kvp.Value.MappingType), typeof(string))));
                    }
                    catch (Exception)
                    {
                        codeBuilder.Append(String.Format(@"
