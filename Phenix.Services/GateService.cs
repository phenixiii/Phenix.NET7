...
namespace Phenix.Services
{
    ...
    public class GateService : IGateService
    {
        ...
        public void PushDynamicPassword(User user, string dynamicPassword, int validityMinutes)
        {
            /*
             * 以下代码供你自己测试用
        ...
        /// <param name="dynamicPassword">动态口令(6位数字一般作为验证码用短信发送给到用户)</param>
        /// <param name="validityMinutes">动态口令有效周期(分钟>=3)</param>
        public void PushPassword(User newUser, string initialPassword, string dynamicPassword, int validityMinutes)
        {
            /*
             * 以下代码供你自己测试用
             * 生产环境下，请替换为通过第三方渠道（邮箱或短信）推送给到用户
