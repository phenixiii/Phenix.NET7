...
namespace Demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("   “_ID”且是长整型15位以上精度：主键/外键；如为主键，新增记录时自动填充Sequence.Value；每张表都应该有且仅一个长整型15位以上精度的主键字段，外键分物理外键（组合关系）和虚拟外键（聚合关系），命名尽可能与主键相呼应；");
            ...
            Console.WriteLine("   “_ORIGINATE_TEAMS”且是字符串/长整型15位以上精度：新增记录时自动填充Identity.CurrentIdentity.User.RootTeams.Name/Id；增删改查记录时自动对分库路由、对单库切片；");
