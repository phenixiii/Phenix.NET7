...
namespace Demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("当有注册用户申请时，你的服务应调用方法 New() 新增用户，然后将调用返回的初始登录口令 {0} 或动态口令 {1} 利用第三方渠道（邮箱或短信）推送给到用户。", password, dynamicPassword);
            Console.WriteLine("被持久化的用户信息，登录口令和动态口令都已自动做了MD5散列：{0}、{1}。", user.Password, user.DynamicPassword);
            Console.WriteLine("用户的手机 {0}、邮箱 {1}、注册昵称 {2}，这些属性，在注册后仍然是可以赋值被自动提交持久化的，前提是你必须保证系统已经成功认证了用户身份才能允许修改。", user.Phone, user.EMail, user.RegAlias);
            ...
            user = User.Fetch("林冲");
            Console.WriteLine("你的服务在响应用户登录时，首先根据传来的用户名获取用户资料，以便完成下一步的身份认证：{0}", Utilities.JsonSerialize(user));
            Console.WriteLine("请按任意键继续");
            Console.ReadKey();
            Console.WriteLine();

            Console.WriteLine("演示核对登录口令有效性的方法 IsValidPassword()");
            Console.WriteLine("你的服务在做用户身份认证时，客户端除了传来用户名，也要提供登录口令（你必须在客户端完成MD5散列化）。");
            Console.WriteLine("身份认证结果：{0}", user.IsValidPassword(MD5CryptoTextProvider.ComputeHash(password), "127.0.0.1", false));
            ...
            Console.WriteLine("用户登录后，服务在每次响应客户端的请求时都要做身份认证，具体实现可以交给 WebAPI 服务框架来完成，在此略过。");
            Console.WriteLine("如果同一用户名在尝试多次身份认证都失败的话，是有怀疑服务在被高频试错攻击，应该禁止这个用户名在一段时间内的登录请求。");
            ...
            Console.WriteLine("你的服务可以响应用户修改登录口令的请求，调用方法 ChangePassword() 传入登录口令（需MD5散列化）和新登录口令（明码）。");
            string newPassword = "123456";
            Console.WriteLine("类似 {0} 这样的简单口令是被禁止的：{1}", newPassword, user.ChangePassword(MD5CryptoTextProvider.ComputeHash(password), newPassword, "127.0.0.1", false) == false ? "ok" : "error");
            ...
            Console.WriteLine("你的服务除了可以通过核对登录口令的有效性来认证用户身份外，还可以通过核对动态口令的方法。");
            Console.WriteLine("身份认证结果：{0}", user.IsValidDynamicPassword(MD5CryptoTextProvider.ComputeHash(dynamicPassword), "127.0.0.1", false));
