...
namespace Demo.IDOS.Plugin.Api.OnlineBooking
{
    ...
    public sealed class InBookingNoteController : Phenix.Core.Net.Api.ControllerBase
    {
        ...
        [HttpGet]
        public async Task<DobInBookingNote> Get(long depotId, string bookingNumber, string licensePlate)
        {
            DobInBookingNote result = await ClusterClient.Default.GetGrain<IInBookingGrain>(depotId).GetNote(bookingNumber, licensePlate);
        ...
        [Authorize(ApiRoles.BookingPerson)]
        [HttpDelete]
        public async Task Cancel([FromBody] DobInBookingNote note)
        {
            await AuthorizationFilters.CheckCustomerUserValidity(User.Identity, note.CmId);
            await ClusterClient.Default.GetGrain<IInBookingGrain>(note.DpId).CancelNote(note.BookingNumber);
        }
    }
}
